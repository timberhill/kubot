...
class Config:
    ...
    def _gather_bot_data(self) -> None:
        ...
        def _gather_subreddit_list(
            lists: list, list_name: str, history: list) -> Tuple[list, list]:
            ...
            subreddit_data = next((
                ...
                if item.get("name") == list_name), None)
            ...
            if list_name in history or \
                list_name in subreddit_data.get("inherit", []):
                raise KubotDispatcherConfigError(
            ...
            for inherit_name in subreddit_data.get("inherit", []):
                history, inherited_subreddits = \
                    _gather_subreddit_list(lists, inherit_name, history)
